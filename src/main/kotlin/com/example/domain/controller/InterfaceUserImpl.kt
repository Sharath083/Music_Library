...
class InterfaceUserImpl(): InterfaceUser {
    ...
    override suspend fun userLoginCheck(name:String, password:String): TokenResponse {
        ...
        return if(name==user?.name && password== user.password){
            val token=methods.tokenGeneratorUser(name)
            ...
            if (!checkSongInPlayList(details.song,details.playList) ) {
                DatabaseFactory.dbQuery {
                    PlayListTable.deleteWhere {
                        songId eq getSongId(details.song) and (userId eq usersId) and(PlayListTable.playListName eq details.playList)
                    }
                }
